...
class Cleaner {
    ...
    public static function ajax_start_job() {
        ...
        $batch = isset( $settings['transient_batch'] ) ? (int) $settings['transient_batch'] : 500;
            ...
            switch ( $job['stage'] ) {
                ...
                case 'transients':
                    ...
                    if ( $deleted < $batch ) {
                        ...
                        $job['progress'] = min( 90, $job['meta']['transients_deleted'] / max(1, $batch) * 5 + 10 );
                    }
    ...
    public static function run_all_blocking() {
        ...
        do {
            $deleted = self::delete_transients_batch( (int) get_option( self::SETTINGS_OPTION )['transient_batch'] ?? 500 );
    ...
    private static function rrmdir_contents( $dir ) {
        if ( ! is_dir( $dir ) ) {
            return false;
        }
        $items = scandir( $dir );
